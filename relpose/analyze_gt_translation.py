...
magnitudes (||t_rel||) across all BEDLAM2 evaluation sequences.

Usage:
    cd eval
    python relpose/analyze_gt_translation.py [--save-csv] [--output-dir DIR]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze GT pairwise relative translation distribution"
    )
    parser.add_argument(
        "--bedlam2-dir",
    ...
    parser.add_argument(
        ...
        help="Save per-pair translation norms to CSV",
    )
    parser.add_argument(
        "--output-dir", default=".",
    ...
    # ── 2. Iterate over sequences and collect pairwise ||t_rel|| ──────
    all_norms = []       # flat list of all ||t_rel||
    ...
    for seq_name in tqdm(dataset.sequence_list, desc="Processing sequences"):
        ...
        t_rel = relative_pose[:, :3, 3]          # (num_pairs, 3)
        norms = torch.norm(t_rel, dim=1).numpy()  # (num_pairs,)

        all_norms.append(norms)
        per_seq_stats.append((seq_name, N, len(norms), norms.mean(), np.median(norms)))

    if skipped:
        print(f"Skipped {skipped} sequences (< 2 frames after sampling)")

    all_norms = np.concatenate(all_norms)
    print(f"\nTotal pairwise translations collected: {len(all_norms)}")
    print(f"From {len(per_seq_stats)} sequences\n")

    # ── 3. Summary statistics ─────────────────────────────────────────
    percentiles = [5, 25, 50, 75, 95]
    pcts = np.percentile(all_norms, percentiles)

    print("=" * 55)
    print("  GT Pairwise Relative Translation ||t_rel|| (meters)")
    print("=" * 55)
    ...
    try:
        ...
        ax.hist(all_norms, bins=100, edgecolor="black", linewidth=0.3, alpha=0.8)
        ax.set_xlabel("||t_rel|| (meters)")
        ax.set_ylabel("Count")
        ax.set_title(
            f"GT Pairwise Relative Translation Distribution\n"
        ...
        hist_path = os.path.join(output_dir, "gt_translation_distribution.png")
    ...
    # ── 5. Optional CSV ───────────────────────────────────────────────
    if args.save_csv:
        csv_path = os.path.join(output_dir, "gt_translation_stats.csv")
        # rebuild with pair info
        import csv
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["seq_name", "pair_i", "pair_j", "t_rel_norm"])
            offset = 0
