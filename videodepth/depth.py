...
EVAL_DEPTH_METADATA = {
    ...
    "gamegeo": {
        "depth_read_func": lambda filename: depth_read_gamegeo(filename),
        "depth_evaluation_kwargs": {
            "max_depth": None,
            "use_gpu": True,
            # "post_clip_max": 70
    ...
    "gamevideo": {
        "depth_read_func": lambda filename: depth_read_gamevideo(filename),
        "depth_evaluation_kwargs": {
            "max_depth": None,
            "use_gpu": True,
            # "post_clip_max": 70
...
_DROID_MAX_DEPTH = 10.0
EVAL_DEPTH_METADATA["droid"] = {
    "depth_read_func": lambda filename: read_droid_depth_png(filename),
    "depth_evaluation_kwargs": {
        "max_depth": _DROID_MAX_DEPTH,
        "use_gpu": True
        # 其他参数按你 kitti/bonn 那个条目复制即可（比如 clip、valid mask 等）
    },
}

EVAL_DEPTH_METADATA["hoi4d"] = {
    "depth_read_func": lambda filename: read_hoi4d_prior_depth_png(filename),
    "depth_evaluation_kwargs": {
        "use_gpu": True,
        "max_depth": None,
    },
...
def read_hoi4d_prior_depth_png(path: str) -> np.ndarray:
    """
    HOI4D prior_depth: PNG (often uint16).
    返回 float32 (H, W). 0 视为 invalid（置为 0）。
    """
    ...
    # 常见情况：uint16 直接存值（0=invalid）
    # 如果你的 prior_depth 实际是 “归一化到 0~65535”，也没关系，scale/shift 对齐会吃掉尺度差
    d[d <= 0] = 0.0
    return d

def read_droid_depth_png(path: str) -> np.ndarray:
    d = imageio.imread(path).astype(np.float32) / 65535.0 * _DROID_MAX_DEPTH
    # invalid -> 0，让你现有的 eval mask 逻辑去处理
    d[(d <= 0) | (d >= _DROID_MAX_DEPTH)] = 0.0
    return d
...
def depth_read_gamegeo(filename: str):
    return np.load(filename) # [H,W]

def depth_read_gamevideo(filename: str):
    return np.load(filename) # [H,W]
