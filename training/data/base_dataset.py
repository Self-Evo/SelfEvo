...
class BaseDataset(Dataset):
    ...
    def process_one_image_wild(
        ...
        # Move principal point to the image center and crop if necessary
        image, _, intri_opencv, _ = crop_image_depth_and_intrinsic_by_pp(
            image, None, intri_opencv, aug_size, filepath=filepath,
        )
        ...
        # Resize images and update intrinsics
        if self.rescale:
            image, _, intri_opencv, _ = resize_image_depth_and_intrinsic(
                image, None, intri_opencv, target_shape, original_size,
                safe_bound=safe_bound,
                rescale_aug=self.rescale_aug
            )
        else:
        ...
        # Ensure final crop to target shape
        image, _, intri_opencv, _ = crop_image_depth_and_intrinsic_by_pp(
            image, None, intri_opencv, target_shape, filepath=filepath, strict=True,
        )
