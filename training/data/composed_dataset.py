...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        # ---- distractor (optional) ----
        has_dis = ("distractor_images" in batch)

        if has_dis:
            if len(batch["distractor_images"]) > 0:
                dis_images = torch.from_numpy(np.stack(batch["distractor_images"]).astype(np.float32)).contiguous()
                dis_images = dis_images.permute(0, 3, 1, 2).to(torch.get_default_dtype()).div(255)
            else:
                # keep shape consistent for collate (0 frames)
                dis_images = torch.zeros((0, 3, images.shape[-2], images.shape[-1]),
                                        dtype=images.dtype, device=images.device)

            dis_ids = torch.from_numpy(batch["distractor_ids"]).to(torch.int64)
            dis_seq_name = batch.get("distractor_seq_name", "")

        # --- Apply Color Augmentation (training mode only) ---
        if self.training and self.image_aug is not None:
            images_ori = images.detach().clone()
            if has_dis:
                dis_images_ori = dis_images.detach().clone()

            if self.cojitter and random.random() > self.cojitter_ratio:
                if has_dis and dis_images.shape[0] > 0:
                    all_imgs = torch.cat([images, dis_images], dim=0)
                    all_imgs = self.image_aug(all_imgs)
                    images = all_imgs[: images.shape[0]]
                    dis_images = all_imgs[images.shape[0] :]
                else:
                    images = self.image_aug(images)
            else:
                for i in range(len(images)):
                    images[i] = self.image_aug(images[i])
                if has_dis:
            ...
            if self.cojitter and random.random() > self.cojitter_ratio:
                if has_dis and dis_images_aug2.shape[0] > 0:
                    all_imgs2 = torch.cat([images_aug2, dis_images_aug2], dim=0)
                    all_imgs2 = self.image_aug(all_imgs2)
                    images_aug2 = all_imgs2[: images_aug2.shape[0]]
                    dis_images_aug2 = all_imgs2[images_aug2.shape[0] :]
                else:
                    images_aug2 = self.image_aug(images_aug2)
            else:
                for i in range(len(images_aug2)):
                    images_aug2[i] = self.image_aug(images_aug2[i])
                if has_dis:
                    for i in range(len(dis_images_aug2)):
                        dis_images_aug2[i] = self.image_aug(dis_images_aug2[i])

        else:
            images_ori = images
            images_aug2 = images
            if has_dis:
                dis_images_ori = dis_images
                dis_images_aug2 = dis_images
        ...
        if has_dis:
            sample["distractor_seq_name"] = dis_seq_name
            sample["distractor_ids"] = dis_ids
            sample["distractor_images_aug"] = dis_images
            sample["distractor_images"] = dis_images_ori
